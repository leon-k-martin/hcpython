...
def main(
    ...
    if overlap1file is not None and overlap2file is not None:
    ...
    if len(bvals1) != overlap1.sum(axis=0)[1] or len(bvals2) != overlap2.sum(axis=0)[1]:
...
def loadFile(filename, ndims, dimlens):
    ...
    if ndims == 2 and len(data.shape) == 1:
        try:
            ...
            raise ValueError(
                "Unexpected size: {0} (expected: {1}, is: "
...
def extract_overlaps(
    bvals1, bvecs1, bvals2, bvecs2, overlap1, overlap2, only_combine=False
):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in (7, 9):
        print(
            "usage: average_bvecs bvals1 bvecs1 "
            "bvals2 bvecs2 "
